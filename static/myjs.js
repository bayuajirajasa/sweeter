                                              ...
                                              <div class="media-content">
                                                  ...
                                                  <nav class="level is-mobile">
                                                      <div class="level-left">
                                                          ...
                                                          <a class="level-item is-sparta" aria-label="heart" onclick="toggle_heart('${post["_id"]
                        }', 'heart')">
                                                              <span class="icon is-small"><i class="fa ${class_heart}"
                                                                                             aria-hidden="true"></i></span>&nbsp;<span class="heart-num">${num2str(
                            post[
                            "count_heart"
                            ]
                        )}</span>
                                                          ...
                                                          <a class="level-item is-sparta" aria-label="star" onclick="toggle_star('${post["_id"]
                                                                                              ...
                                                                                              <span class="icon is-small"><i class="fa ${class_star}"
                                                                                                                             aria-hidden="true"></i></span>&nbsp;<span class="star-num">${num2str(
                            post[
                            "count_star"
                            ]
                        )}</span>
                                                                                          </a>
                                                                                          <a class="level-item is-sparta" aria-label="thumbs" onclick="toggle_like('${post["_id"]
                        }', 'thumbs')">
...
function toggle_heart(post_id, type) {
    console.log(post_id, type);
    let $a_heart = $(`#${post_id} a[aria-label='heart']`);
    let $i_heart = $a_heart.find("i");
    if ($i_heart.hasClass("fa-heart")) {
        $.ajax({
            type: "POST",
            url: "/update_heart",
            data: {
            ...
            success: function (response) {
                console.log("unheart");
                $i_heart.addClass("fa-heart-o").removeClass("fa-heart");
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log("heart");
                $i_heart.addClass("fa-heart").removeClass("fa-heart-o");
                $a_heart.find("span.heart-num").text(num2str(response["count"]));
            },
        });
    }
