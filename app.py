...
app.config['UPLOUD_FOLDER'] = '/static/profile_pics'
...
@app.route("/")
def home():
    token_receive = request.cookies.get("SPARTA")
    try:
...
def get_posts():
    ...
    try:
        ...
        for post in posts:
            ...
            post["count_heart"] = db.hearts.count_documents(
        ...
        for post in posts:
            ...
            post["thumbs_by_me"] = bool(
                db.likes.find_one(
                    {"post_id": post["_id"], "type": "thumbs", "username": payload["id"]}
                )
            )
...
@app.route("/update_heart", methods=["POST"])
def update_heart():
    token_receive = request.cookies.get("SPARTA")
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        # We should change the heart count for the post here
        user_info = db.users.find_one({"username": payload["id"]})
        ...
        if action_receive == "heart":
            db.hearts.insert_one(doc)
        else:
            db.hearts.delete_one(doc)
        count = db.hearts.count_documents(
...
def update_star():
    ...
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        # We should change the star count for the post here
        user_info = db.users.find_one({"username": payload["id"]})
        post_id_receive = request.form["post_id_give"]
        type_receive = request.form["type_give"]
        action_receive = request.form["action_give"]
